...
### 観測電圧 `v_cmd`（ソフト指令）との整合性

同定の観測電圧は電流ループの**電圧指令 `v_cmd`**（ソフト値）。「ECU が電流を誤認するなら `v_cmd` は真の印加電圧でなく不整合では?」への回答:

- 定常で `v_cmd` は **`i_sw` ではなく `i_true` と整合する**。電流センサ誤差は「どの `i_true` に着地するか」を変えるだけで、PI 積分が `i_sw=i_cmd` を満たすまで動いた `v_cmd` は**物理的に `i_true` を駆動している電圧そのもの**だから。`v_applied = v_cmd − v_dist`（`v_dist`=インバータ歪み）を物理式に代入すると:
...
これは同定式そのもので、**電流センサ誤差はこの式に現れない**（i_true をどう実現したかには非依存）。

- **不整合が出るのは `(v_cmd, i_sw)` を組にした時だけ**。校正後 `i_true` と組めば整合。**実務ルール: 校正 `i_true` は「マップの出力座標」だけでなく、電圧式の `Rs·i` 項と `psi` の引数にも同じ `i_true` を使う**（座標だけ直して回帰に `i_sw` を使うと不整合が残る）。
- `v_cmd ≠ v_applied` は事実だが真因は `v_dist`（インバータ歪み）で電流誤差ではない。`v_dist` は回帰の**切片**に落ち、柱 C で分離 → **ψ（傾き）には乗らない**。
- 標準手法（Armando 2013）も定常で**指令電圧**を使いインバータ誤差を別補正。PI 積分が誤デカップリング/FF を吸収するので定常 `v_cmd` は頑健。電圧飽和域だけ `v_cmd` がクランプされ乖離 → G1 の変調限界内取得で回避。

### 電圧スケール誤差 `K_v` は別軸（電流プローブでは直らない）

`v_applied = K_v·v_cmd`（Vdc 認識誤差・PWM スケール）は **ψ＝傾きを直接 ×K_v でバイアス**する。これは電流側の校正では捕まらない**電圧側**の誤差で、電流の共通ゲイン `K` と**対**をなす（`K`＝電流振幅、`K_v`＝電圧振幅＝ψ を直接スケール）。ψ 精度の床はこの2つで決まる。

- 対処: Vdc 計測精度の確認。必要なら電圧プローブで `K_v` を1点校正。
- 絶対ロータ角オフセットも別軸だが、`v` と `i` を同じだけ回すのでフレーム内では整合（マップが回るだけ）。
...
- **電圧スケール `K_v`（別軸）**: `v_applied = K_v·v_cmd`（Vdc/PWM 由来）は ψ＝傾きを直接バイアス。電流プローブでは直らない電圧側の項。Vdc 計測精度を確認し、必要なら電圧プローブで1点校正（詳細は Phase 0 校正節）。
