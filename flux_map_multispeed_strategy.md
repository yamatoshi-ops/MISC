...
| 取得列 | `id, iq, vd, vq, omega_e(or rpm), torque, 温度` | 温度は熱ドリフト監視（ψm, Rs） |
...
| Phase | 内容 | 主成果物 | 依存 | 状態 |
|---|---|---|---|---|
| 0 | 取得計画確定（電流点・速度段・変調限界 feasibility・符号/単位 lock・ゲート定義） | acquisition plan note | 既存 VI データ（feasibility 外挿） | 未 |
