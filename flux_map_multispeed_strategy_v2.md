...
## 電圧誤差について
鋭い指摘です。これは「焦ったら止まれ」で表面の現象（VdVq がソフト値）と真因を二段で切り分けるべき所なので、丁寧に詰めます。

結論から: **不整合が生じるのは「v_cmd と i_sw（未校正）を組にした場合」だけ**です。校正後の i_true と組にすれば整合します。理由は物理にあります。

## なぜ v_cmd は i_sw ではなく i_true と整合するのか

定常状態でモータが従う真の物理（電力不変 dq、真の角度フレーム）:

```text
v_applied,d = Rs·id_true − omega_e·psi_q(i_true)
v_applied,q = Rs·iq_true + omega_e·psi_d(i_true)
```

ここに2つの事実を入れます:

1. **電流ループは i_sw を i_cmd に追い込む** → 物理的に流れているのは i_true（= i_sw を K・τ で戻した真電流）。電流センサ誤差は「**どの i_true に着地するか**」を変えるだけ。
2. **インバータ**: `v_applied = v_cmd − v_dist`（v_dist = デッドタイム等のインバータ歪み）。

この2つを代入すると:

```text
vd_cmd = Rs·id_true − omega_e·psi_q(i_true) + v_dist,d
vq_cmd = Rs·iq_true + omega_e·psi_d(i_true) + v_dist,q
```

→ **これは同定式そのもの**で、`v = v_cmd`, `i = i_true`, `e = v_dist`。**電流センサ誤差はこの式のどこにも現れません**。v_cmd は「PI 積分が i_sw=i_cmd を満たすまで動いた結果」で、それは物理的に **i_true を駆動している電圧そのもの**だからです。制御が電流を誤認していても、モータは嘘をつかない — v_cmd は常に実電流 i_true と整合します。

つまりあなたの違和感の正体は「**v_cmd を i_sw と組にすると、v_cmd は実は i_true の電圧なのに座標が K・τ ずれた i_sw に貼り付く**」という不整合。**電流校正（i_true 復元）は、まさにこの不整合を消すための操作**です。

## ただし、ここが実務上の新しい気づき

電流校正 i_true は「**マップの出力座標**」だけでなく、**電圧式の中の `Rs·i` 項と `psi` の引数にも同じ i_true を使う**必要があります。座標ラベルだけ直して回帰には i_sw を使う、では不整合が残ります。校正は「電圧式に入る i」に効かせて初めて整合します。

## あなたの「v_cmd ≠ 真の印加電圧」は正しい — ただし真因は電流ではない

`v_cmd ≠ v_applied` は事実です。が、その差は **v_dist（インバータ歪み）** であって、電流センサ誤差ではありません。そして v_dist は:

- multi-speed 回帰では**切片**に落ちる（速度非依存）→ 柱 C で `Rs·i + e(i)` として分離・LUT 化、というのが既に計画済みの処理。
- だから「v_cmd が生の印加電圧でない」分は、**ψ（傾き）には乗らず**、ちゃんと別項として扱われます。

## 一方で、本当に別物として残る懸念（正直に2点）

| 懸念 | 中身 | 電流プローブで直る？ | 対処 |
|---|---|---|---|
| **電圧スケール誤差 K_v** | `v_applied = K_v·v_cmd`（Vdc 認識誤差・PWM スケール）。**ψ=傾きを直接 ×K_v でバイアス** | **直らない**（電流側の話ではない） | Vdc 計測精度の確認。必要なら電圧プローブで K_v を1点校正 |
| **電圧飽和** | 変調限界超で v_cmd がクランプ→印加と乖離 | — | G1 の変調限界内取得で回避（既に計画内） |

K_v は ψ 精度の床を決めるので、電流側の K と**対**で確認すべき項目です（電流プローブだけでは捕まらない）。絶対ロータ角オフセットも別軸ですが、これは v と i を同じだけ回すのでフレーム内では整合（マップが回るだけ）。

## まとめ

- **不整合は無い** — ただし v_cmd と組にするのは **i_true（校正後）**、かつ電圧式の `Rs·i`・`psi` 引数にも i_true を使う、という条件下で。
- これは標準手法（Armando 2013 も定常で**指令電圧**を使い、インバータ誤差を別補正する）と同じ理屈。PI 積分が誤デカップリングを吸収するので**定常の v_cmd は頑健**。
- 電流校正で残らない懸念は **電圧スケール K_v**（別途・電圧側）。

この「整合性の根拠（v_cmd ⇄ i_true）」と「電圧スケール K_v は別軸」を戦略書の Phase 0 校正節に注記として足しておきますか? 後で同じ疑問が再燃しないよう、残すと効くと思います。
